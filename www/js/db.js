$.Class('CDB', {
	init: function() {
		...
		if (device.platform == 'Android') {
			this.db = window.openDatabase('domoteka', '', 'Application Database', 10485760); 
		} else if (device.platform == 'chrome') {
			this.db = window.openDatabase('domoteka', '', 'Application Database', 10485760); 
		} else {
			this.db = window.sqlitePlugin.openDatabase({name: 'domoteka.db', iosDatabaseLocation: 'default'});
		}
